...
class PPOModel(object):
    ...
    def create_visual_encoder(self, o_size_h, o_size_w, bw, h_size, num_streams, activation, num_layers, ch = None):
        """
        ...
        for i in range(num_streams):
            self.conv1 = tf.layers.conv2d(self.observation_in[-1], 8, kernel_size=[4, 4], strides=[1, 1],
                                          use_bias=True, activation=activation)
            self.conv2 = tf.layers.conv2d(self.conv1, 16, kernel_size=[4, 4], strides=[2, 2],
                                          use_bias=True, activation=activation)
...
class ContinuousControlModel(PPOModel):
    def __init__(self, lr, brain, h_size, epsilon, max_step, normalize, num_layers):
        ...
        if brain.number_observations > 0:
            ...
            height_size, width_size = 20, 20
            encoders.append(self.create_visual_encoder(height_size, width_size, False, h_size, 2, tf.nn.tanh, num_layers, ch=7))
            hidden_visual = tf.concat(encoders, axis=2)
