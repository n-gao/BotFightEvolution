...
public class CoreBrainInternal : ScriptableObject, CoreBrain
{
    ...
    public void SendState()
    {
        ...
        float[,,,] data = new float[agentKeys.Count, 20, 20, 7];
        for(int i = 0; i < agentKeys.Count; i++) {
            for (int j = 0; j < 20; j++) {
                for (int k = 0; k < 20; k++) {
                    for (int c = 0; c < 7; c++) {
                        data[i, j, k, c] = inputState[i, j * 140 + k * 7 + c];
                    }
                }
            }
        }
        var tmp = inputState;
        inputState = new float[currentBatchSize, 8];
        for (int i = 0; i < agentKeys.Count; i++) {
            for (int j = 0; j < 8; j++) {
                inputState[i, j] = tmp[i, 2800 + j];
            }
        }
        observationMatrixList.Add(data);
    ...
    public void DecideAction()
    {
        ...
        // Create the observation tensors
        for (int obs_number = 0; obs_number < 1; obs_number++)
        {
    ...
    public void OnInspector()
    {
        ...
        if (brain.brainParameters.cameraResolutions != null)
        {
                ...
                {
                    ObservationPlaceholderName = new string[1];
                }
                if (ObservationPlaceholderName.Count() != 1)
                {
                    ObservationPlaceholderName = new string[1];
                }
                for (int obs_number = 0; obs_number < 1; obs_number++)
